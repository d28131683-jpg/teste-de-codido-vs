...
def recommendations(
    ...
    return {'items': get_repo().list_recommendations(segment=segment, sport=sport, limit=limit)}
...
def history(
    ...
    return {'items': get_repo().list_history(status=status, limit=limit)}


@router.get('/api/v1/admin/status')
def admin_status() -> dict:
    return get_repo().admin_status()


@router.get('/api/v1/admin/logs')
def admin_logs(limit: int = Query(default=100, ge=1, le=500)) -> dict:
    return {'items': get_repo().list_logs(limit=limit)}
